...
class Stimulus:
    ...
    def __init__(self):
        ...
        if par['normalize01']:
            print("NORMALIZING")
            print(self.training_data[0].mean())
            self.training_data = np.float32(self.training_data)
            self.training_output = np.float32(self.training_output)
            self.training_data   /= 255.0
            self.training_output /= 255.0
            self.testing_data    = np.float32(self.testing_data)/255
            self.testing_output  = np.float32(self.testing_output)/255
            print(self.training_data[0].mean(), self.training_data[0].mean()*255)
