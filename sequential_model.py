...
class EvoModel:
    ...
    def run_model(self):
        ...
        self.output = tf.nn.relu(tf.reshape(logits, [par['batch_train_size'],par['n_output']]))
        # conv1  = tf.layers.conv2d(inputs=x, filters=3, kernel_size=(3,3), padding='same', activation=tf.nn.relu)
        # self.output = tf.nn.relu(tf.reshape(conv1, [par['batch_train_size'],par['n_output']]))
